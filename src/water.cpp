...
public:
    ...
    constexpr size_t operator()(const VesselsState& state) const noexcept {
        return (size_t(state[0])*std::numeric_limits<water>::max()
                + state[1])*std::numeric_limits<water>::max()
                + state[2];
    };
...
protected:
    ...
    void show(const water target, int steps) {
        ...
        fmt::print("Solved measure {} liters of water using {}, {} and {} vessels in {} steps\n",
                target, m_volumes.at(0), m_volumes.at(1), m_volumes.at(2), steps);
        fmt::print("┌──────┬─────┬─────┬─────┐\n");
        fmt::print("│ Step │ {: >3} │ {: >3} │ {: >3} │\n", m_volumes.at(0), m_volumes.at(1), m_volumes.at(2));
        fmt::print("├──────┼─────┼─────┼─────┤\n");

        // If only the first solution is needed we can modify the history to reverse the index pointers and walk forward.
